...
#   6. Copies TeX4ht CSS from html_temp/output/ to static/css/
#
...
# --- Python helper: strip height= attributes from <img> tags ---
...
def fix_img(m):
    # Remove height="..." or height='...' from inside the matched <img> tag
    return re.sub(r"""\s*height\s*=\s*(?:"[^"]*"|'[^']*')""", '', m.group(0), flags=re.IGNORECASE)

html = sys.stdin.read()
html = re.sub(r'<img\b[^>]*>', fix_img, html, flags=re.IGNORECASE | re.DOTALL)
...
convert_project() {
    ...
    # ------------------------------------------------------------------
    # Step 5: Extract <body> content and fix image aspect ratios
    #
    # make4ht sets explicit width="..." and height="..." pixel attributes
    # on every <img> tag. When the Hugo theme's CSS scales the image by
    # width (e.g. max-width: 100%), the hardcoded height attribute is not
    # overridden and forces a distorted, often square, display.
    #
    # Fix: strip all height="..." attributes from <img> tags so the
    # browser derives the height from the image's natural aspect ratio
    # and the rendered width — exactly as a browser normally would.
    # ------------------------------------------------------------------
    local html_body
    ...
    # ------------------------------------------------------------------
    # Step 8: Copy TeX4ht CSS from html_temp/output/ to static folder
    #
    # make4ht generates a stylesheet (e.g. main.css) that handles
    # indentation, spacing, and other TeX-specific layout rules.
    # ------------------------------------------------------------------
    local css_count=0
    for css in "$out_subdir"/*.css; do
        if [[ -f "$css" ]]; then
            cp "$css" "$STATIC_DIR/"
            (( css_count++ )) || true
        fi
    ...
    [[ $css_count -gt 0 ]] && \
        echo -e "  ${GREEN}↳ CSS${NC}        : $css_count file(s) copied to '$STATIC_DIR/'"
...
if [[ "$MODE" == "single" ]]; then
    ...
    echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD} convert.sh — LaTeX → Hugo HTML (single)${NC}"
    echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD} convert.sh — LaTeX → Hugo HTML (batch)${NC}"
    echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
